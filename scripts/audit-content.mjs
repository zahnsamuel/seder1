...
// Also importable: loadUnits(root), analyze(unit), auditAll(root) back the
// test/content-standard-floor.test.mjs guard so the corpus cannot silently
// regress below the score-8 standard.
...
export function loadUnits(root = '.') {
  ...
  // --- Tractate labs ---
  const labs = JSON.parse(readFileSync(join(root, 'data/tractate-labs.json'), 'utf8'));
  ...
  for (const cf of courseFiles) {
    const data = JSON.parse(readFileSync(join(root, `data/${cf}.json`), 'utf8'));
...
export function auditAll(root = '.') {
  return loadUnits(root).map(analyze).sort((a, b) => a.score - b.score);
}

// CLI: print the table + distribution when run directly (node scripts/audit-content.mjs).
if (process.argv[1] && process.argv[1].replace(/\\/g, '/').endsWith('scripts/audit-content.mjs')) {
