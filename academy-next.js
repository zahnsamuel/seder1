...
function render(learner) {
  ...
  $('#recommendation').innerHTML = `<h2 id="recommendation-title">Next: ${next.label}</h2><p>${next.copy}</p><p class="capability-line">${Seder.capabilitySentence(counts)}</p><p><b>This group still has the least secure evidence in your record.</b> Make it secure in familiar sources, then transferable in an unfamiliar one. This is a recommendation, not a verdict.</p><a class="primary" href="${next.url}">Begin this next move -></a><a href="weekly-review.html">Retrieve before moving on -></a>`;
