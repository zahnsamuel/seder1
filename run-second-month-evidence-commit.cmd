@echo off
cd /d "%~dp0"
echo Staging second-month source-specific Academy evidence...
git add -- academy-evidence.js test/academy-mastery-gate.test.mjs docs/qa-intake.md
git diff --cached --check || goto :error
git commit -m "Add source-specific second-month academy evidence" || goto :error
git push origin main || goto :error
echo.
echo Success. Second-month Academy evidence is committed and pushed.
pause
exit /b 0
