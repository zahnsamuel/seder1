...
function renderCompletion(collection) {
  ...
  $('#reader-completion').innerHTML = complete ? `<strong>Passage complete</strong><p>You have worked through every line and recorded your reading path.</p><a class="jla-btn jla-btn-primary" href="${collection.connectionUrl}">Carry this source into the next unit &rarr;</a><a class="source-link" href="daily-router.html">Return to today&apos;s plan &rarr;</a>` : `<strong>Close the reading loop</strong><p>${ready ? 'You have focused every line. Name the move that mattered most, then complete this passage.' : `Focus ${collection.lines.length - viewed.size} more line${collection.lines.length - viewed.size === 1 ? '' : 's'} before closing the passage.`}</p><textarea id="reading-reflection" placeholder="What did this source ask you to notice?"></textarea><button id="complete-reading" class="save-note jla-btn jla-btn-primary" ${ready ? '' : 'disabled'}>Complete this passage</button>`;
...
function render() {
  ...
  $('#reader').innerHTML = collection.lines.map((line, lineIndex) => `<article class="source-line" data-line="${lineIndex}"><div class="line-ref">${line.ref}</div><p class="line-hebrew" lang="he" dir="rtl">${line.hebrew}</p><p class="line-translation" hidden>${line.translation}</p><div class="line-actions"><button class="translate" type="button">Show translation</button><button class="focus-line" type="button">Focus this line</button><button class="highlight" type="button">Highlight</button><a href="notebook.html?source=${encodeURIComponent(line.ref)}">Open in notebook</a></div><p class="line-note">${line.note}</p><textarea placeholder="Private note on this line"></textarea><button class="save-note jla-btn jla-btn-ghost" type="button">Save line note</button></article>`).join('') + '<section id="reader-completion" class="source-line"></section>';
