...
| **Key-prerequisite remediation** | Each KP links to the *key* prerequisite it most directly uses; fail twice at a KP → auto remedial review of that key prerequisite. | **Built (proposed links)** — every KP proposes its key prerequisite (introduce → most foundational, practice/transfer → most proximate), `pending-expert`; the hook for KP-level remediation. |
