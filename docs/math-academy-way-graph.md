...
| **Key-prerequisite remediation** | Each KP links to the *key* prerequisite it most directly uses; fail twice at a KP → auto remedial review of that key prerequisite. | **Built + wired, KP-granular** — answers carry a `knowledgePointId`; the repository tracks `knowledgePointStruggles` per KP; `keyPrerequisiteRemediation` routes a KP failed ≥ twice to *that KP's* key prerequisite (introduce → foundational, practice/transfer → proximate), naming the step ("the practice step of X"), and falls back to skill-level when no KP is known. The academy-session flow tags its graded check as the practice KP. |
...
| **Diagnostic → frontier estimate** | An adaptive placement exam estimates the frontier; refined as lessons complete. | **Built + wired into placement** — `estimateFrontierFromDiagnostic` + `nextDiagnosticProbe` (`data/knowledge-graph.mjs`), served at `POST /api/graph/diagnostic` (pins the exact frontier in ~8–21 questions, not 49). Every `placement_completed` is enriched server-side by downward inference (`enrichPlacementWithFrontier`): demonstrating one mid-layer skill seeds its whole prerequisite chain as secured, so the learner starts at their true frontier — verified: 1 demonstrated skill → 12 secured, recommended the next frontier move. |
