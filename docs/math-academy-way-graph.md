...
| **Scaffolded mastery learning** | Each topic = a lesson of **knowledge points** (worked example + questions), basic → advanced; master each KP to unlock more advanced topics. | **Partial** — each skill has `teachingMove` + `checks`; KPs as first-class scaffolded sub-steps are not yet modeled. |
| **Key-prerequisite remediation** | Each KP links to the *key* prerequisite it most directly uses; fail twice at a KP → auto remedial review of that key prerequisite. | **Partial** — remediation exists (`remediationFor`), but is struggle-count based, not KP→key-prereq. |
...
| **Diagnostic → frontier estimate** | An adaptive placement exam estimates the frontier; refined as lessons complete. | **Partial** — placement exists and gates recommendations; it is not yet a graph-frontier estimator. |
...
2. **Knowledge points** — decompose each skill into scaffolded KPs (basic → advanced), each with its
   key prerequisite, to enable MA-style KP-level mastery and targeted remediation.
