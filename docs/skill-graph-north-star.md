...
not a content catalog and not a reading of someone elseΓÇÖs book.
...
Jewish Learning Academy takes an adult from Jewish-learning insecurity to **0ΓåÆ1
...
The emotional win is **love of Jewish learning** ΓÇö ease in non-Haredi Jewish space,
...
A **skill** is a transferable *reading capability* ΓÇö a move the learner can perform
...
is not a skill ΓÇö it is a lesson.
...
| A **concept** or topic (ΓÇ£the Oven of AknaiΓÇ¥, ΓÇ£free willΓÇ¥) | Concepts are things sources talk about. Skills are things learners *do*. |
...
| A **Yochai corpus node** | That graph answers ΓÇ£where is this in the library?ΓÇ¥ Ours answers ΓÇ£what can you do now?ΓÇ¥ |

The `graph:skills` gate already fails the build if a foundation node is named after
a source. Keep that line.

## Scope: 0ΓåÆ1, not Shas, not a few thousand topics
...
- **GemaraΓÇÖs job:** a powerful laboratory for question, case, evidence, dispute,
...
references. The shell must not invent a second ΓÇ£next step.ΓÇ¥ The learner should
...
reviewing, or repairing (`fnd-ΓÇª`). A title and a URL without a skill are a page
recommendation, not a skill recommendation.

## Evidence, not streaks-as-truth

Capability states (from the graphΓÇÖs own scale):
...
A running 0ΓÇô1 score, XP, or a single multiple-choice hit is **instrumentation**,
not the product language. Advance on evidence. Retrieve before the skill fades.
Do not treat ΓÇ£opened the pageΓÇ¥ as mastery.
...
YochaiΓÇÖs knowledge graph is a **source substrate** ΓÇö a way to find, cluster, and
...
If the two graphs ever share identifiers, the skill id remains `fnd-ΓÇª` and the
...
|---|---|---|
| `data/foundation-skill-graph.json` | Capability DAG (`fnd-ΓÇª`) | **The brain.** Placement, frontier, daily teaching, graduation. |
...
| `data/content-skill-graph.mjs` + `non-gemara-skill-graph.mjs` | One node per authored source move (~850) | Content index. May tag `fnd-ΓÇª`; must not *be* the frontier. |
...
are mapped. Legacy graphs are not consulted for ΓÇ£what should the learner do now?ΓÇ¥
...
1. **Point Today at a frontier `fnd-` skill, not back at Today.** Done (PR #13).
   ...
   (Layer 0 ΓåÆ `hebrew-decoding.html`). `skillId` is on the public next-action
   contract.
...
2. **Replace the hardcoded 14-skill ladder with `knowledgeFrontier()`.** Done (PR #13).
   ...
   key prerequisite ΓÇö exclusively on `fnd-` ids.
...
3. **Stop letting the content-move graph choose the next action.**
   ...
   chosen, it looks up `foundation-content-map` and returns ΓÇ£practice this skill
   in a real unit.ΓÇ¥ Content graphs stay indexes.

4. **Name a teachable starter set and freeze the rest.**
   Named and frozen (`data/foundation-starter-set.json`, 29 teach / 26 hold).
   TodayΓÇÖs teach / review / repair now consume that list: `knowledgeFrontier`
...
5. **One placement, one id space.**
   ...
   translation layer only ΓÇö they are not the learnerΓÇÖs ΓÇ£start hereΓÇ¥ id.

6. **Retrieval checks on the same ids.**
   Done (PR #18). Review, decay, and welcome-back recall cite `fnd-` skills
   and open `academy-session.html?skill=<fnd-id>` (Layer 0 ΓåÆ `hebrew-decoding.html`).
   The check is an authored item when present, else a graph-derived retrieval or
   a mapped real-source step ΓÇö not a generic Daf prompt and not a vanished
   content-step id.
...
9. **Educator pass on the starter edges, then item banks.**
   ...
   Software cannot invent those. Stage the audit workbench; write ΓëÑ1 real
...
3. **Stop letting the content-move graph choose the next action.**
   ...
   returns ΓÇ£practice this skill in a real unit.ΓÇ¥ Content graphs stay indexes.
...
4. **Name a teachable starter set and freeze the rest.**
   ...
   Layer 0 decode banks (4 skills) are still held ΓÇö `hebrew-decoding.html`
...
5. **One placement, one id space.**
   ...
   (`source-family-001`) remain a translation layer only ΓÇö they are not the
   learnerΓÇÖs ΓÇ£start hereΓÇ¥ id.
...
7. **Close the fat buckets before adding siblings.**
   ...
   proposal for one child (`fnd-arg-resolve-distinction`) is PR #32 ΓÇö not
...
9. **Educator pass on the starter edges, then item banks.**
   ...
   Software cannot invent the rationales. Write ΓëÑ1 real transfer item per
...
4. **Name a teachable starter set and freeze the rest.**
   Live graph: 55 skills, 11 layers. Item banks are 0/55. Pick ~20ΓÇô30 skills the
...
A live scan of every other open PR (#30ΓÇô#42) found **no second clearly
...
9. **Educator pass on the starter edges, then item banks.**
   ...
   labeled EXAMPLE stubs). Write ΓëÑ1 real transfer item per starter skill before expanding.
