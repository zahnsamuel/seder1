...
4. **Name a teachable starter set and freeze the rest.**
   Done (PR #14 freeze; PR #30 router). Live graph: 55 skills, 11 layers, frozen
   as 29 starter / 26 hold (`data/foundation-starter-set.json`). Today's teach /
   review / repair consume that list: `knowledgeFrontier` can scope to the slice,
   and `pickFrontierFoundationSkill` will not surface a frozen sibling while
   starter skills remain unsecured. Authored banks exist for 25/29 starter skills
   (78 items in `data/foundation-authored-items.json`) via PRs #16 / #22 / #27 /
   #28; teach-before-ask is product law (PR #29). Layer 0 decode banks (4 skills)
   are still held — `hebrew-decoding.html` is the on-ramp. Remaining work is
   educator-audit + transfer items (item 9), not more skills. Do not grow toward
   150 until those fill. `npm run graph:quality` still reports 0/55 first-class
   assessment banks; that counter reads `foundation-assessment-items.json`, not
   the authored JSON above.
...
7. **Close the fat buckets before adding siblings.**
   Proposal landed as docs, not a live split (PR #32). `fnd-arg-response`,
   `fnd-role-ruling-vs-discussion`, and `fnd-case-what-happens` still swallow
   most tagged content. The judgment is in
   [`docs/fat-bucket-first-split.md`](fat-bucket-first-split.md) and
   `data/fat-bucket-first-split.json`: one justified child
   (`fnd-arg-resolve-distinction`) — **not** on the live 55-skill graph. Split
   only where the daily loop would route differently, and only with a rubric
   signal plus retagged units
   ([`docs/foundation-graph-growth-plan.md`](foundation-graph-growth-plan.md)).
   Do not implement the live split until that proposal is accepted into the DAG.

8. **Capability-state language on the learner surfaces that already show
   progress.** Done (PR #25 path / academy / academy-next; PR #34 leftovers).
   Learner copy uses emerging / secure / transferable / durable on path,
   academy references, and leftover Today-adjacent / practice / progress /
   placement chrome. Today stays one action, not a scoreboard. Hidden `#xp`
   hooks on lesson arcs remain instrumentation.

9. **Educator pass on the starter edges, then item banks.**
   Staging landed empty (PR #37 workbench; PR #40 transfer schema). Rationales
   are still 0/76; named misconceptions 0/55; authored transfer items 0/55
   (`npm run graph:quality`). [`educator-edge-audit.html`](../educator-edge-audit.html)
   lists the 29 starter skills and 38 closed edges against
   [`data/foundation-edge-audit.json`](../data/foundation-edge-audit.json)
   (empty but for labeled EXAMPLE stubs). Transfer contract is staged (schema
   + 2 EXAMPLE items, not counted, not loaded):
   [`docs/foundation-transfer-items.md`](foundation-transfer-items.md).
   Starter recognition banks exist (item 4) but are not transfer items. Software
   cannot invent the rationales. Write ≥1 real transfer item per starter skill
   before expanding.
