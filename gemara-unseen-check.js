...
$('#check').onclick = async () => {
  ...
  await Seder.api(`/api/learners/${learnerId}/events`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'answer_submitted', skillId: 'unseen-sugya-reading', competency: 'argument', sourceContext: `Unseen Gemara: ${item[0]}`, correct }) }).catch(() => {});
};
