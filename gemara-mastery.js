...
Seder.api(`/api/learners/${learnerId}`).then((response) => response.ok ? response.json() : null).then((learner) => {
  ...
  const stateFor = (tractate) => {
    ...
    const complete = [map, second, transfer, scheduled].filter(Boolean).length;
  ...
  const finishedBefore = (cohortIndex) => records.slice(0, cohortIndex).every((cohort) => cohort.records.every(({ state }) => state.transfer));
  let recommended = null;
  records.forEach((cohort, cohortIndex) => cohort.records.forEach((record, index) => {
    record.available = finishedBefore(cohortIndex) && (index === 0 || cohort.records[index - 1].state.transfer);
    if (!recommended && record.available && !record.state.transfer) recommended = { record, cohort, cohortIndex };
  }));
  ...
  $('#tractates').innerHTML = records.map((cohort, cohortIndex) => {
    ...
    const earned = cohort.records.filter(({ state }) => state.transfer).length;
    return `<section class="cohort ${cohortOpen ? '' : 'locked-cohort'}"><header><div><small>${cohort.title}</small><p>${cohort.promise}</p></div><strong>${earned} / ${cohort.records.length} transfers</strong></header>${cohort.records.map(({ tractate, state }, index) => {
      const available = finishedBefore(cohortIndex) && (index === 0 || cohort.records[index - 1].state.transfer);
      const locked = !available && !state.transfer;
      const currentIndex = !state.map ? 0 : !state.second ? 1 : !state.transfer ? 2 : state.due ? 3 : -1;
      const gate = locked ? `Complete ${index ? cohort.records[index - 1].tractate.title : records[cohortIndex - 1].title} first to open this move.` : state.transfer ? 'Evidence earned; keep it durable through retrieval.' : 'This is available now.';
      return `<article class="tractate ${locked ? 'locked' : ''}"><div class="tractate-head"><div><h2>${tractate.title}</h2><p>${tractate.focus}</p></div><span class="level">${locked ? 'UPCOMING' : labelFor(state.complete)}</span></div><p class="gate">${gate}</p><div class="stages">${stageCard('First source mapped', state.map, !locked && currentIndex === 0, 'Map the visible opening source.')}${stageCard('Second source explained', state.second, !locked && currentIndex === 1, 'Compare the deeper argument.')}${stageCard('Transfer demonstrated', state.transfer, !locked && currentIndex === 2, 'Read the move on a new page.')}${stageCard('Retrieval scheduled', state.scheduled, !locked && currentIndex === 3, state.due ? 'A short retrieval is due now.' : 'Schedule follows source-map completion.')}${stageCard('Durable next step', state.transfer && !state.due, false, 'Continue the mastery loop.')}</div>${locked ? '<span class="next disabled">Upcoming after evidence →</span>' : `<a class="next" href="${state.next.url}">${state.next.label} →</a>`}</article>`;
    }).join('')}</section>`;
  }).join('');
