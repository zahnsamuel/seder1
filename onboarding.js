(() => {
  ...
  window.showOnboardingIfNew = (hasProgress) => {
    ...
    panel.innerHTML = `<p>WELCOME</p><h2>Learn to read Jewish sources yourself.</h2><p class="onboarding-purpose">Short daily practice that builds real skill — you prove you can make the reading move, not collect points. Today is one sitting: a name, a few checks, one lesson.</p><ol><li><b>1. Pick a name.</b> No email, no password — just something to save your progress.</li><li><b>2. Answer a few questions.</b> At most six short checks — not a quiz. We’ll start you in the right place.</li><li><b>3. Do one short lesson.</b> See it on the page, then answer. That’s the whole first visit.</li></ol><div><a href="sign-in.html?next=diagnostic.html">Start learning →</a><button type="button">Got it</button></div>`;
