(function () {
  ...
  const fallback = { title: 'Continue today’s learning', reason: 'One short lesson is waiting.', href: 'daily-router.html', cta: 'Start this lesson' };
  const lastSkill = (() => { try { return sessionStorage.getItem('jla-last-foundation-skill'); } catch { return null; } })();
  const FIRST_HINT = 'You’ll see a short source, then answer. Then you’ll come back here.';
  const NEXT_HINT = 'That’s the next short lesson if you want it.';
  ...
  const render = (action) => {
    ...
    const foundationLike = item.type === 'foundation' || /start this (lesson|step)/i.test(item.cta || '');
    const returning = Boolean(lastSkill && item.skillId && lastSkill !== item.skillId && foundationLike);
    link.textContent = `${returning ? 'Start the next lesson' : (item.cta || fallback.cta)} →`;
    link.href = safeHref(item.href || fallback.href); link.className = 'jla-next-action__cta';
    ...
    hint.textContent = returning ? NEXT_HINT : FIRST_HINT;
