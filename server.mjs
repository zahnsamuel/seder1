...
async function handleApi(request, response, url) {
  ...
  // complete:true when the frontier is pinned or the first-day probe cap is hit. Only skills with
  // an authored item are probed; L0 decode is skipped (no invented glyph banks).
  if (request.method === 'POST' && url.pathname === '/api/graph/diagnostic') {
    ...
    const probeId = nextDiagnosticProbe(graph, responses, { probeable, maxProbes: DIAGNOSTIC_PROBE_CAP });
