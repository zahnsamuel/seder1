...
test('Yoma has a dedicated visible-source workspace from case through proof-text response', async () => {
  const [html, source, engine, sequences] = await Promise.all([
    'yoma-daf-workbench.html', 'yoma-daf-workbench.js', 'course-engine.js', 'data/gemara-source-sequences.json'
  ...
  assert.match(html, /href="gemara-year\.html">Gemara Year/);
  assert.match(engine, /'yoma-tractate-arc':\{tractate:'yoma',url:'yoma-daf-workbench\.html'/);
