...
test('graph-quality report runs and covers the five layers', () => {
  ...
  // It reports honest gaps, not a green wash: the typed-edge structure is built (76/76 — 73 after the
  // transitive-reduction cleanup, +2 from the authored intermediate skill), but the pedagogical
  // rationales stay unwritten until the educator audit — that gap must still show.
  assert.match(out, /edges typed .*76\/76/);
  assert.match(out, /pedagogical rationale .*0\/76/);
});
