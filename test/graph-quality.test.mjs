...
test('graph-quality report runs and covers the five layers', () => {
  ...
  // It reports honest gaps, not a green wash: the typed-edge structure is built (75/75 — 73 after the
  ...
  assert.match(out, /edges typed .*75\/75/);
  assert.match(out, /pedagogical rationale .*0\/75/);
});
