...
test('daily router covers six repair categories and protects the placement-first entry',async()=>{const [js,whyNext]=await Promise.all(['daily-router.js','why-next.js'].map((file)=>readFile(new URL(`../${file}`,import.meta.url),'utf8')));assert.equal(router.categories.length,6);assert.ok(router.categories.every(x=>x.skills.length&&x.url));assert.match(js,/const needsPlacement = !learner\.placement/);assert.match(js,/needsPlacement\) recommendation = \{ title: 'Find your starting point', url: 'diagnostic\.html'/);assert.match(whyNext,/if \(!learner\?\.placement\) return/);assert.match(js,/Resume \$\{active\.course\.title\}/);assert.match(js,/canon-capstone\.html\?course/);});
...
test('daily learning and the server recommendation agree on the current Foundation Year term', async () => {
  const [router, server, terms, foundationYear] = await Promise.all([
    readFile(new URL('../daily-router.js', import.meta.url), 'utf8'),
    readFile(new URL('../server.mjs', import.meta.url), 'utf8'),
    readFile(new URL('../data/term-recommendations.mjs', import.meta.url), 'utf8'),
  ...
  for (const stage of ['foundation-capstone', 'term-two-capstone', 'second-foundation-synthesis']) {
    ...
    assert.match(terms, new RegExp(stage));
  ...
  assert.match(terms, /export function foundationRecommendation/);
...
test('daily learning and the server recommendation advance through Gemara Year one tractate at a time', async () => {
  const [router, server, terms, year] = await Promise.all([
    readFile(new URL('../daily-router.js', import.meta.url), 'utf8'),
    readFile(new URL('../server.mjs', import.meta.url), 'utf8'),
    readFile(new URL('../data/term-recommendations.mjs', import.meta.url), 'utf8'),
  ...
  assert.match(terms, /export function gemaraYearRecommendation/);
...
test('daily learning opens the earned Moed Expansion only after Gemara Year', async () => {
  const [router, server, terms, expansion] = await Promise.all([
    readFile(new URL('../daily-router.js', import.meta.url), 'utf8'),
    readFile(new URL('../server.mjs', import.meta.url), 'utf8'),
    readFile(new URL('../data/term-recommendations.mjs', import.meta.url), 'utf8'),
  ...
  assert.match(terms, /export function moedExpansionRecommendation/);
