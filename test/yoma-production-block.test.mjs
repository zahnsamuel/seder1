...
test('Yoma connects procedure, limit, proof text, and transfer through the visible-source workspace', async () => {
  const [arc, page, recordsFile] = await Promise.all([
    'yoma-arc.js', 'yoma-arc.html', 'data/yoma-source-review.json'
  ...
  assert.match(arc, /nextUrl:'yoma-daf-workbench\.html'/);
  assert.match(arc, /nextLabel:'Open the Yoma source workspace'/);
