...
// After the L1/L2 4→5 thicken (PR #62), the next remaining 4-item starter banks
// are L3 roles, L4 cases, the first L5 argument pair, plus L7 genre and L8
// responsibility. Each needs ≥5 valid, teach-before-ask-clean items and a fair See-it.
const THICKENED = [
  'fnd-role-example',
...
test('the next twelve 4-item starter banks have ≥5 valid items and fair See-it teach', async () => {
...
test('example See-it still names the concrete-case shapes the checks rely on', async () => {
  const teach = (await load('data/foundation-teach.json')).teach['fnd-role-example'];
  assert.match(teach, /someone who/i);
  assert.match(teach, /ox that/i);
  assert.match(teach, /booth that is higher than/i);
});
