...
test('study record collects learner-owned evidence from durable and local study artifacts', async () => {
  const [html, source, landing] = await Promise.all(['study-record.html', 'study-record.js', 'seder.js'].map((file) => readFile(new URL(`../${file}`, import.meta.url), 'utf8')));
  ...
  assert.match(landing, /study-record\.html/);
});
