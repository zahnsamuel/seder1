...
function answer(button, correct, item) {
  if (answered) return; answered = true; document.querySelectorAll('#answers button').forEach((btn) => { btn.disabled = true; }); button.classList.add(correct ? 'correct' : 'incorrect');
...
function render() {
  ...
  const answers = $('#answers'); answers.innerHTML = ''; shuffledAnswers(item).forEach(({ text, index }) => { const button = document.createElement('button'); button.type = 'button'; button.textContent = text; button.addEventListener('click', () => answer(button, index === item.correct, item)); answers.appendChild(button); });
}
