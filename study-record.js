...
Promise.all([Seder.api(`/api/learners/${learnerId}`).then((response) => response.ok ? response.json() : null), Seder.api(`/api/learners/${learnerId}/review`).then((response) => response.ok ? response.json() : { due: [], upcoming: [] })])
  .then(([learner, review]) => {
    ...
    $('#summary').textContent = `${Seder.capabilitySentence(counts)} You have ${academyDays} Academy day${academyDays === 1 ? '' : 's'} with evidence and ${evidenceCount} source context${evidenceCount === 1 ? '' : 's'} recorded. This record grows through evidence, not screen completion.`;
    $('#metrics').innerHTML = [['Capabilities', Seder.capabilityEyebrow(counts)], ['Academy days with evidence', academyDays], ['Due reviews', review.due.length], ['Independent readings', independent]].map(([name, value]) => `<article><small>${name}</small><strong>${value}</strong></article>`).join('');
    $('#skills').innerHTML = list([...established.slice(0, 5).map(([id]) => `<li><span>${label(id)}</span><b class="state secure">Secure</b></li>`), ...fading.slice(0, 3).map(([id]) => `<li><span>${label(id)}</span><b class="state fading">refresh soon</b></li>`)], 'Demonstrate a source move to begin your record.');
