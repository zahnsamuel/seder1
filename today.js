...
Seder.api(`/api/learners/${learnerId}/today`).then((response) => response.ok ? response.json() : Promise.reject()).then((plan) => {
  ...
  document.querySelector('#session-copy').textContent = plan.steps.length === 1 ? 'Begin with a short placement so Jewish Learning Academy can shape future sessions around what you actually know.' : 'A focused sequence of retrieval, new learning, and a clear return to your path.';
